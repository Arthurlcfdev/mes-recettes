...
export async function findRecipeBySourceUrl(sourceUrl) {
  if (!sourceUrl) return null;
  const target = normalizeUrl(sourceUrl);
  const rows = await sql`SELECT id, name, source_url, deleted_at FROM recipes WHERE source_url IS NOT NULL`;
  const match = rows.find((row) => normalizeUrl(row.source_url) === target);
  return match ? { id: match.id, name: match.name, deletedAt: match.deleted_at } : null;
}
...
export async function insertRecipe({
  ...
  const data = { description, servings, prepTime, cookTime, ingredients, steps, image };
